...
int main()
{
	...
	int k[10] = { adventurer, gardens, embargo, village, minion, mine, cutpurse,
		...
		for (int i = 0; i < TESTNUM; i++) {
			...
			int result = cardEffect(village, 1, 1, 1, &G, 0, &bonus);
			
			fFlag += asserttrue(result, 0, 0);
			fFlag += asserttrue(statG.playedCardCount, (G.playedCardCount + 1), 1);
			fFlag += asserttrue(G.handCount[G.whoseTurn], statG.handCount[statG.whoseTurn] + 1, 2);
			fFlag += asserttrue(G.numActions, statG.numActions + 2, 3);
